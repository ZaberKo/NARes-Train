...
class MOTradesLoss(nn.Module):
    def __init__(self, step_size=0.007, epsilon=0.031, perturb_steps=10, beta=6.0,
                 distance='l_inf', ce=False, cutmix=False, adjust_freeze=True, normalization_type='none'):
        super(MOTradesLoss, self).__init__()
        ...
        self.num_tasks = 2  # natural loss + robust regularization
    ...
    def forward(self, model, x_natural, y, optimizer):
        ...
        loss_data[0] = loss_natural.data[0]
        grads[0] = []
        for param in model.parameters():
        ...
        loss_data[1] = loss_robust.data[0]
        grads[1] = []
        for param in model.parameters():
            if param.grad is not None:
        ...
        sol, min_norm = MinNormSolver.find_min_norm_element([grads[t] for t in range(self.num_tasks)])
        for t in range(self.num_tasks):
            scale[t] = float(sol[t])
        ...
        loss = scale[0] * loss_natural + scale[1] * loss_robust
